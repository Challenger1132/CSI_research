%% Runs the SpotFi test over the passed in data files which each contain CSI data
% for many packets
...
function output_top_aoas = spotfi(csi_trace, frequency, sub_freq_delta, antenna_distance, data_name)
    ...
    if OUTPUT_AOA_VS_TOF_PLOT && ~OUTPUT_SUPPRESSED && ~OUTPUT_FIGURES_SUPPRESSED
        ...
        for ii = 1:size(cluster_indices, 1)
            ...
            if isempty(cluster_indices{ii})  %cell
                continue
            end
            ...
            plot(full_measurement_matrix(cluster_indices{ii, 1}, 1) * aoa_max, ...
                    ...
                    'LineWidth', 2.5)
			% 以上代码画出的是第ii个聚类的效果
			% 不用考虑图像叠加在一起吗？因为采用的聚类，不同的AOA以及TOF被聚类到一起
			% 每个聚类本身就具有不同的均值和方差
			% 画出AOA以及TOF的平面聚类效果是一个一个的簇
            % Compute Means
...
% 一堆点，如何绘制一个椭圆包围这一堆点
% 求一堆点的质心做圆心，求数据的元素最大差值的一般做半径，绘制椭圆
...
function [estimated_aoas, estimated_tofs] = aoa_tof_music(x, ...
    ...
    for ii = 1:size(eigenvalue_matrix, 1)
        if eigenvalue_matrix(ii, ii) > max_eigenvalue
            max_eigenvalue = eigenvalue_matrix(ii, ii); % 最大特征值
        end
    end
    ...
    eigenvectors = eigenvectors(:, column_indices); % 噪声空间
    % Peak search
    ...
    for ii = 1:length(theta) % 对于每个θ和tau都构造一个导向矢量
    ...
    % 返回binary_peaks_pmusic 和Pmusic维度相同 
	% 找Pmusic的区域最大值，返回二值图像，像素设为1代表区域最大值，其他值为0
	% 对于2-D图像，默认是8连通的
	...
	% 对矩阵按行或按列进行，如若包含非0元素返回1反之返回0
    estimated_aoas = theta(aoa_peaks_binary_vector); % 取出aoa_peaks_binary_vector不为0对应θ
    ...
    for ii = 1:length(aoa_peak_indices)
		...
		% binary_peaks_pmusic是logical的那么取出某一行也是logical的
