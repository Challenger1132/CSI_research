...
function [aoa_packet_data,tof_packet_data,output_top_aoas] = spotfi(csi_trace, frequency, sub_freq_delta, antenna_distance, data_name)
    ...
    for ii = 1:size(clusters, 1)  % 遍历每个cluster
        ...
        alpha = 0.05;  %显著性水平
        [~, outlier_indices, ~] = deleteoutliers(clusters{ii}(:, 1), alpha); %删除AOA异常值
		%返回异常值在 clusters{ii}(:, 1) 中的下标
        cluster_indices{ii}(outlier_indices(:), :) = []; % cluster_indices{ii}  (第ii个聚类)中异常值所在的行被删掉
        clusters{ii}(outlier_indices(:), :) = [];  %异常值从cluster中移除

        alpha = 0.05;
        [~, outlier_indices, ~] = deleteoutliers(clusters{ii}(:, 2), alpha); %删除TOF异常值
        cluster_indices{ii}(outlier_indices(:), :) = [];
        clusters{ii}(outlier_indices(:), :) = [];
    end
	%----------%
	% 每一个聚类用不同的颜色进行表示，绘制图像
	%----------%
	% 在聚类之前对full_measurement_matrix矩阵中的元素进行了归一化，对应值都除以了aoa_max、tof_max
	% 然后进行聚类
	% 接着进行异常值剔除
	% 下面求AOA均值的时候都要乘以aoa_max
    ...
    for ii = 1:length(clusters)  % 聚类数
        ...
        aoa_variance = aoa_variance / (num_cluster_points - 1);  % 采用num_cluster_points - 1是无偏估计量
        ...
        % cluster_aoa(ii, 1) = aoa_mean * aoa_max;
