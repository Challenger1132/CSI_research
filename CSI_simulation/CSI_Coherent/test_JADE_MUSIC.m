...
3、导向矢量的构建要和流型矩阵“对齐”
4、相干信号与非相干信号的产生方式
非相干信号：Gamma = randn(paths,samples)+randn(paths,samples)*1j;
相干信号：产生paths-coherentPaths的非相干信号，再将信号的第一行乘以一个随机数追加到矩阵中
产生paths*samples的矩阵	[Gamma_temp; ComplexConst*repmat(Gamma_temp(1,:), CoherentPaths, 1)];
导向矢量乘以复衰减再添加噪声
X = A*Gamma; % M*paths * paths*samples = M*samples
CSI = awgn(X, SNR, 'measured'); % M*samples
5、相干非相干指的是复衰减系数？与导向矩阵没有关系？
